...
python -m ohe  \
  --file_in csvs/covid_11_pivot_death_uniform_D.csv \
  ...
  --file_out csvs/covid_11_pivot_death_uniform_D_OHE.csv

python -m predict  \
  --file_in csvs/covid_11_pivot_death_uniform_D_OHE.csv \
  --strategy none \
  ...
  --score recall \
  --file_out csvs/covid_11_pivot_death_uniform_D_OHE_PREDICT.csv