...
python -m ohe  \
  --file_in csvs/covid_10_pivot_death_dbscan_D.csv \
  ...
  --file_out csvs/covid_10_pivot_death_dbscan_D_OHE.csv

python -m predict  \
  --file_in csvs/covid_10_pivot_death_dbscan_D_OHE.csv \
  --strategy none \
  ...
  --score recall \
  --file_out csvs/covid_10_pivot_death_dbscan_D_OHE_PREDICT.csv