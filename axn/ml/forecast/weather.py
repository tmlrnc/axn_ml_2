...
python %s -q <zipcode> -n <stations per location> -d <preferred distance km>''' % tuple([sys.argv[0]] * 2)
    ...
    if not query and (cfgFile is None or outFile is None): print
    instruction;
    sys.exit()
...
Using %d stations per location and a preferred distance of %d km.''' % (cfgFile, outFile, n, prefDist)
    ...
    if query:
        ...
        for stationData in sList: print
        stationData
        sys.exit()
    ...
    with open(outFile, 'wb') as outFile:
        ...
        for key in sorted(monthCfg.keys()):
            print
            'Working on month %s' % key
            zips = monthCfg[key]
            monthStack = wd.weatherMonth(zips, key.year, key.month, hourly=True, n=n, preferredDistKm=prefDist,
                                         stackData=True)
            ...
            for zip5 in zips:
                ...
                print
                '  Writing %s. %d rows.' % (zip5, len(flat))
            ...
            print
            'Elapsed time: ', datetime.datetime.now() - startTime
